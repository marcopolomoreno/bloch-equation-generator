function downloadCodigoFonteTempoMisturaOndas()
{
...
function delta()
{
    ...
    for (var k=0; k<=N-1; k++)
        for (var p=k+1; p<=N-1; p++)
            string = string + "\tdouble delta" + String(k+1) + String(p+1) + " = 0;\n"

    string = string + "\n"

    for (var k=N-1; k>=0; k--)
        for (var p=k-1; p>=0; p--)
            string = string + "\tdouble delta" + String(k+1) + String(p+1) + " = - delta" + String(p+1) + String(k+1) + ";\n"
...
`
    ...
    //Intervalo entre pontos no gráfico, em unidades de h
    int const dt = 100;
...
function printArquivo()
{
    ...
    //Unidades
    string = string + `\tfprintf(arquivo, "ns`
...
cont[17] = `);
    ...
    var nomeArquivo = "evolucao-temporal-mistura-ondas.c";
